...
func _process(delta):
	...
	if player_enter_alo and Input.is_action_just_pressed("e") and not camera_switched:
		...
		if is_instance_valid(alo_camera):
			...
			$"../InnervoiceLamba".visible = true
			$"../InnervoiceLamba/InnerVoice/AnimationPlayer".play("Fade_up")
...
func _on_vue_lamba_body_entered(body):
	if body.is_in_group("player"):
		player_enter_alo = true


func _on_vue_lamba_body_exited(body):
	if body.is_in_group("player"):
