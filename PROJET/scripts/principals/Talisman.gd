...
const ROTATION_SPEED := 0.1  # Vitesse de rotation ajustable
...
func rotate_camera(mouse_delta: Vector2):
	...
	rotation.x -= mouse_delta.y * ROTATION_SPEED * 0.01  # Inversé pour une sensation plus naturelle
	suz.rotation.y -= mouse_delta.x * ROTATION_SPEED * 0.01

	# Limiter l'inclinaison verticale pour éviter un retournement
